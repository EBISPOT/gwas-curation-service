...
import uk.ac.ebi.spot.gwas.deposition.junidecode.Junidecode;
...
public class EuropePMCDeserializer extends StdDeserializer<EuropePMCData> {
    ...
    public ArrayList<PublicationAuthorDto> getAuthorsInfo(JsonNode info) {
        ArrayList<PublicationAuthorDto> authors = new ArrayList<>();
        ...
        for (JsonNode author : authorList) {
            PublicationAuthorDto newAuthor = new PublicationAuthorDto();
            String fullname;
            if (author.has("collectiveName")) {
                fullname = author.get("collectiveName").asText();
                newAuthor.setFullName(fullname);
                newAuthor.setFullNameStandard(Junidecode.unidecode(fullname));
            } else {
                noAuthor = false;
        ...
        if (noAuthor) {
            if (root.has("investigatorList")) {
                ...
                for (JsonNode author : investigatorList) {
                    ...
                    getLog().info(newAuthor.getFullName());
                    if (author.has("authorId")) {
                        newAuthor.setOrcid(author.get("authorId").get("value").asText());
                    }
                    authors.add(newAuthor);
    ...
    public PublicationDto getPublicatonInfo(JsonNode info) {
        JsonNode root = info.get("resultList").get("result").get(0);
        ...
        return PublicationDto.builder().
                pmid(root.get("pmid").asText())
                .journal(root.get("journalInfo").get("journal").get("medlineAbbreviation").asText())
    ...
    public EuropePMCData deserialize(JsonParser jp, DeserializationContext ctxt)
        ...
        if (node.get("resultList").get("result").size() > 0) {
            ...
            ArrayList<PublicationAuthorDto> listAuthors = getAuthorsInfo(node);
            record.setAuthors(listAuthors);
            PublicationAuthorDto firstAuthor = listAuthors.get(0);
